...
export default {
    ...
    methods: {
        ...
        getAllComments () {
            api.getAllComments(this.$route.params.id).then(response => {
        ...
        addLike (id, index) {
            ...
            if (i === -1) {
                api.updateLike(id, {option: 'add'}).then(() => {
                ...
                this.updateLike(id, {option: 'drop'}).then(() => {
    ...
    watch: {
        $route (to, from) {
            ...
            this.getAllComments(to.params.id)
        }
    }
}
